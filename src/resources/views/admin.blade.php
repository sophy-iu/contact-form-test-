...
<script>
    function openModal(id) {
        fetch(`/admin/contact/${id}`)
            ...
            .then(data => {
                let gender = '';
                if (data.gender == 1 || data.gender === 'male') gender = '男性';
                else if (data.gender == 2 || data.gender === 'female') gender = '女性';
                else gender = 'その他';
                ...
                document.getElementById('modal-body').innerHTML = `
                    ...
                    <form method="POST" action="/admin/contact/${data.id}" onsubmit="return confirm('本当に削除しますか？')">
                        @csrf
                        @method('DELETE')
